...
import type {
  GraphNode, GraphLink, GraphMetadata, AILink,
  CrossGraphStats, AILinkStats, WorkspaceSnapshot,
} from '@/types'
import graphDataManager from '../services/GraphDataManager'

// 跨圖譜測試資料（空預設，如需測試資料請在 ../data/ 下建立 crossGraphTestData）
const crossGraphData: { metadata: any[]; aiLinks: any[]; graphs: any[] } = { metadata: [], aiLinks: [], graphs: [] };

export interface CrossGraphDeps {
  nodes: Ref<GraphNode[]>;
  links: Ref<GraphLink[]>;
  loading: Ref<boolean>;
...
export function useCrossGraphFeatures(deps: CrossGraphDeps) {
  const { nodes, links, loading, error, lastUpdate } = deps

  // ===== State =====
  const graphMetadataList: Ref<GraphMetadata[]> = ref([])
  const aiLinks: Ref<AILink[]> = ref([])
  const activeGraphIds: Ref<string[]> = ref([])
  ...
  const graphStats: ComputedRef<CrossGraphStats> = computed(() => ({
  ...
  const loadGraphMetadataList = async (options: { forceRefresh?: boolean } = {}): Promise<GraphMetadata[]> => {
    try {
  ...
  const loadCrossGraphData = async (graphIds: string[] = ['graph-tech', 'graph-learning']) => {
    loading.value = true
    error.value = null
  ...
  const snapshotWorkspaceGraph = (): WorkspaceSnapshot => {
