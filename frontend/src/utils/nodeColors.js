...
/**
 * 根據節點類型獲取顏色配置
 * @param {string} type - 節點類型 (不區分大小寫)
 * @returns {Object} { color, glow, label, emoji, group }
 */
 ...
 */
export function getNodeGlow(node) {
  const config = getNodeTypeColor(node.type);
...
/**
 * 獲取所有已定義的類型列表（供圖例使用）
 * @returns {Array<{type, color, glow, label, emoji, group}>}
 */
 ...
 * @returns {Array<{type, color, glow, label, emoji, count}>}
 */
export function getActiveNodeTypes(nodes) {
  const typeCount = {};
  nodes.forEach(n => {
