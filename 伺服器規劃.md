...
前端提供 **2D (AntV G6)** 與 **3D (Three.js + 3D-Force-Graph)** 雙模式的知識圖譜視覺化介面，支援拖放操作、跨圖譜關聯、智能推薦連結、**圖譜刪除管理**等功能。
...
| 容器化 | Docker Compose (14 個微服務) |
| 本地 LLM | Ollama (llama3 + nomic-embed-text) |
...
> 以下為 BruV Enterprise Server 全功能完整運作圖，涵蓋所有 14 個 Docker 容器、7 條 API 路由、  
> 4 個背景服務、11 個資料儲存、3 組可觀測性堆疊，以及前端 15 個頁面路由。

```
╔══════════════════════════════════════════════════════════════════════════════════════════════════════════════╗
║                                   BruV Enterprise Server v4.0 — 完整運作圖                                  ║
╠══════════════════════════════════════════════════════════════════════════════════════════════════════════════╣
║                                                                                                            ║
║   ┌─────────────────────────────────────────────────────────────────────────────┐                           ║
...
│  ❻ 可觀測性流 (日誌 + 監控)                                                                  │
│  ────────────────────────                                                                    │
│  所有 14 容器 → Docker stdout → Promtail (自動採集) → Loki (聚合) → Grafana (儀表板 :3000)  │
...
### 5.1 主程式進入點 (app_anytype.py, 675 行)
...
| 模組 | 行數 | 功能描述 |
|------|------|---------|
| **graph.py** | 536 | 知識圖譜核心 API — 實體建立、關係管理、鄰居查詢、Cypher 安全驗證、圖譜元資料 CRUD、**圖譜刪除 (級聯)**、**手動節點互連觸發** |
...
| **system.py** | 599 | 系統配置讀寫、連線測試、通用檔案上傳、CircuitBreaker 狀態、Saga DLQ 管理 |
...
使用 Docker Compose 編排 **14 個微服務**，所有服務在 `bruv_network` (bridge) 網路中通訊。
