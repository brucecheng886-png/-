...
**v5.2 新增**：架構深度審查後的全面重構 — **階段一安全強化** (eval→json.loads、移除 URL query 認證、DOMPurify XSS 防護) + **階段二結構拆分** (後端 3 大模組拆分：graph_import 1205→3 檔、system 977→4 檔、watcher 978→3 檔｜前端 graphStore 1590→1012+3 composables、GraphPage 2792→1595+2 元件)。所有拆分維持向後相容、零 consumer 變更。
...
```
╔══════════════════════════════════════════════════════════════════════════════════════════════════════════════╗
║                                   BruV Enterprise Server v5.2 — 完整運作圖                                  ║
╠══════════════════════════════════════════════════════════════════════════════════════════════════════════════╣
║                                                                                                            ║
║   ┌─────────────────────────────────────────────────────────────────────────────┐                           ║
...
**graphStore.js** (1012 行，v5.2 拆分後)：
- 管理知識圖譜的所有狀態：nodes、links、選中節點、2D/3D 模式切換、跨圖譜 AI Link
- **v5.2 Composable 拆分**：將匯入、跨圖譜、RAGFlow 邏輯分離為 3 個 Composable 模組，以依賴注入模式初始化，主 Store 透過展開運算子 `...spread` 組合所有功能，**零 consumer 變更**
  - `importComposable.js` (230 行) — importFile、importExcelAsync、pollImportStatus、uploadFileToGraph 等匯入相關 state + actions
  - `crossGraphComposable.js` (187 行) — graphMetadataList、aiLinks、isCrossGraphMode、loadCrossGraphData、snapshotWorkspaceGraph 等跨圖譜 state/computed/actions
  - `ragflowComposable.js` (28 行) — ragflowDatasets、fetchRAGFlowDatasets
...
**GraphPage.vue** (1595 行，v5.2 拆分後，原 2792 行)：
- 知識圖譜主頁面 — 2D/3D 切換、節點 CRUD、搜尋、篩選、連線管理
- **v5.2 元件拆分**：將兩大面板抽離為獨立元件，減少 ~1200 行、消除死碼
  - `ClusterSettingsDialog.vue` (420 行) — 星系圖片設定面板（Canvas 星球渲染、預設圖庫、URL/檔案上傳）；以 `v-model` 控制顯隱
  - `NodeInspectorPanel.vue` (371 行) — 節點檢查器面板（本地編輯、Tag 管理、AI 建議連線、封面圖片）；透過 `@save`/`@delete`/`@highlight-node` 事件通知父元件
- 事件橋接模式：GraphPage 保留 4 個輕量 handler (`closeInspector`/`handleInspectorSave`/`handleInspectorDelete`/`handleInspectorHighlight`)，委派至 graphStore 統一 API
...
| **graph_import.py** | 205 | 薄路由層 — Excel/CSV 匯入端點定義 *(v5.2 拆分，原 1205 行)* |
| **import_engine.py** | 754 | 匯入核心引擎 — 智能解析、LLM 快取、合併上傳、3000 筆支援 *(v5.2 新增)* |
| **import_prompts.py** | 81 | LLM Prompt v2.0 模板集中管理 *(v5.2 新增)* |
...
| **system.py** | 271 | 薄路由層 — 組合子路由器 *(v5.2 拆分，原 977 行)* |
| **system_config.py** | 106 | 配置工具 & Pydantic 模型 *(v5.2 新增)* |
| **system_upload.py** | 206 | 通用檔案上傳管線 *(v5.2 新增)* |
| **system_connections.py** | 270 | 連線管理 CRUD (6 端點) + 自動偵測服務 *(v5.2 新增)* |
...
**watcher.py** (355 行，v5.2 拆分後，原 978 行) — *v3.4 大幅強化*：
...
- **v5.2 拆分**：核心處理邏輯拆離為 2 個子模組，主檔保留 Observer/Handler/DLQ/重試框架
  - `file_processor.py` (339 行) — Saga 流程 / RAGFlow 上傳 / KuzuDB 元資料寫入 / Excel 深度解析
  - `node_linker.py` (157 行) — 節點互連引擎：Link Domain 歸類 + 關鍵字共現分析
- **節點互連引擎** *(v3.4 新增，v5.2 拆至 node_linker.py)*：
...
│  ├── 401 自動清除 Token + 重定向
│  └── *(v5.2)* 移除 URL query param 認證路徑，僅允許 Header Bearer Token
│
第4層 ─ 業務邏輯
...
│  ├── *(v5.2)* eval() 全面替換為 json.loads()（消除任意程式碼執行風險）
│  └── *(v5.2)* DOMPurify v-html 清理 — DifyChat + AICopilot 的 LLM 回應內容防 XSS
│
第5層 ─ 資料隔離
   ├── 完全地端部署 (零外部雲端依賴)
...
| dompurify | ^3.3.1 | XSS 防護 (v-html 清理) *(v5.2 新增)* |
| vite | ^6.4.1 | 建構工具 |
...
**整體評分：7.2 / 10 → v3.3 後提升至 8.5 / 10 → v5.2 後提升至 9.0 / 10** ✅
...
| 2026-02-17 | v5.2 | **架構深度審查：安全強化 (Stage 1) + 結構拆分 (Stage 2)**。詳見下方 v5.2 變更明細 |
| 2026-02-18 | v5.3 | **架構深度審查：效能優化 (Stage 3)**。詳見下方 v5.3 變更明細 |

### v5.3 變更明細

**核心目標：架構深度審查後的效能優化 (Stage 3) — 消除阻塞式 I/O、共享連線池、減少打包體積**

> 依據 `docs/ARCHITECTURE_REVIEW.md` Stage 3 清單，6 項全數完成。

#### 一、httpx 共享連線池 (P3-1)

| 問題 | 修復 | 影響 |
|------|------|------|
| 23 處 per-request `httpx.AsyncClient()` — 每次請求建立/銷毀 TCP 連線 | 統一使用 `app.state.http_client` 共享連線池 (50 max connections, 10 keepalive) | 大幅降低連線建立延遲、減少 TIME_WAIT socket |

**修改檔案：**
| 檔案 | 變更 |
|------|------|
| `backend/api/dify.py` | 4 個端點改用 `raw_request.app.state.http_client` |
| `backend/api/ragflow.py` | 7 個端點改用共享連線池或傳入 `RAGFlowClient` |
| `backend/rag_client.py` | 新增 `http_client` 參數 + `_get_async_client()` helper，8 個 async 方法改用共享池 |
| `backend/api/graph.py` | `delete_graph` 端點傳入 `http_client` 至 `RAGFlowClient` |
| `backend/api/graph_import.py` | `_run_import()` 傳入 `http_client` |
| `backend/api/system_upload.py` | `upload_file` 端點傳入 `http_client` |

#### 二、graph.py 全面 async 化 (P3-2)

| 問題 | 修復 | 影響 |
|------|------|------|
| 15 個 async 端點直接呼叫同步 KuzuDB 方法，阻塞事件迴圈 | 全部改用 `await kuzu_manager.safe_*()` 非同步方法 (Lock/Semaphore + ThreadPoolExecutor) | 解除事件迴圈阻塞，支援真正的並發處理 |

**修改檔案：**
| 檔案 | 變更 |
|------|------|
| `backend/api/graph.py` | 15 個端點：`add_entity()` → `await safe_add_entity()` 等 |
| `backend/core/kuzu_manager.py` | 新增 `safe_get_entity()`、`safe_update_graph_metadata()` 2 個 async 方法 |

#### 三、auth.py Token 快取 (P3-3)

| 問題 | 修復 | 影響 |
|------|------|------|
| 每次 API 認證觸發 `open()` + `json.load()` 磁碟讀取 | 30 秒 TTL 記憶體快取 + 寫入時主動失效 | 高頻認證場景 I/O 降 30 倍以上 |

**修改檔案：** `backend/core/auth.py`
- 新增 `_token_store_cache`、`_token_store_cache_time`、`_TOKEN_CACHE_TTL = 30.0`、`_token_cache_lock`
- `_load_token_store()` 優先讀快取，過期才讀磁碟
- `_save_token_store()` 寫入後呼叫 `_invalidate_token_cache()`

#### 四、Element Plus 按需引入 (P3-4)

| 問題 | 修復 | 影響 |
|------|------|------|
| `app.use(ElementPlus)` 全量載入 60+ 元件 | `unplugin-vue-components` + `unplugin-auto-import` 按需自動引入 | 初始 bundle 大幅縮減（僅載入實際使用的元件） |

**修改檔案：**
| 檔案 | 變更 |
|------|------|
| `frontend/vite.config.js` | 新增 `AutoImport` + `Components` 插件 (ElementPlusResolver) |
| `frontend/src/main.js` | 移除 `import ElementPlus` 及 `app.use(ElementPlus)`；保留圖標全域註冊 |
| `frontend/package.json` | 新增 `unplugin-vue-components`、`unplugin-auto-import` devDependencies |

#### 五、移除 @antv/g6 死碼 (P3-5)

| 問題 | 修復 | 影響 |
|------|------|------|
| `@antv/g6` 未使用但仍打包 (~1 MB+) | 刪除 `GraphView.vue`、移除路由、移除 dependency | bundle 減少 ~1 MB |

**修改檔案：**
| 檔案 | 變更 |
|------|------|
| `frontend/src/components/GraphView.vue` | **已刪除** (342 行舊版 2D 圖譜) |
| `frontend/src/router/index.js` | 移除 `/graph` 路由 + lazy import |
| `frontend/package.json` | 移除 `@antv/g6` dependency |

#### 六、移除 axios (P3-6)

| 問題 | 修復 | 影響 |
|------|------|------|
| `axios` 已全面替換為 `fetch`，但殘留 dependency (~400 KB) | 移除 `package.json` 中的 `axios` | bundle 減少 ~400 KB |

**修改檔案：** `frontend/package.json` — 移除 `"axios": "^1.13.4"`

#### 修改檔案總覽 (v5.3)

| 類別 | 檔案 | 異動類型 |
|------|------|---------|
| 後端 API | `backend/api/graph.py` | 15 端點 async 化 + httpx 共享池 |
| 後端 API | `backend/api/dify.py` | 4 端點 httpx 共享池 |
| 後端 API | `backend/api/ragflow.py` | 7 端點 httpx 共享池 |
| 後端 API | `backend/api/graph_import.py` | httpx 傳入 |
| 後端 API | `backend/api/system_upload.py` | httpx 傳入 |
| 後端核心 | `backend/core/auth.py` | Token 快取機制 |
| 後端核心 | `backend/core/kuzu_manager.py` | 2 個新 safe_* 方法 |
| 後端客戶端 | `backend/rag_client.py` | 共享連線池支援 |
| 前端建置 | `frontend/vite.config.js` | Element Plus 按需引入插件 |
| 前端入口 | `frontend/src/main.js` | 移除全量載入 |
| 前端路由 | `frontend/src/router/index.js` | 移除 /graph 路由 |
| 前端依賴 | `frontend/package.json` | -@antv/g6, -axios, +unplugin-* |
| 前端元件 | `frontend/src/components/GraphView.vue` | **刪除** |

#### 驗證結果

- ✅ 後端 6 檔案 — 0 errors
- ✅ 前端 3 檔案 — 0 errors
- ✅ Element Plus 圖標全域註冊保留
- ✅ 所有 safe_* 方法使用 Lock (寫) / Semaphore (讀) 保護

---

### v5.2 變更明細

**核心目標：架構深度審查後的安全強化 (Stage 1) + 結構拆分 (Stage 2)**

> 依據 `docs/ARCHITECTURE_REVIEW.md` 審查報告，分兩階段實施。所有拆分維持向後相容、零 consumer 變更。

#### 一、Stage 1 — 安全修復 (4 項)

| # | 風險 | 修復 | 修改檔案 |
|---|------|------|---------|
| S1 | `eval()` 任意程式碼執行 | 替換為 `json.loads()` (安全 JSON 解析) | `backend/services/watcher.py` → 已拆至 `file_processor.py` |
| S2 | URL query param 認證繞過 | 移除 `request.query_params` Token 路徑，僅保留 Header Bearer Token | `backend/core/auth.py` |
| S3 | XSS via v-html (LLM 回應注入) | 導入 DOMPurify (^3.3.1)，`sanitize()` 包裹所有 v-html 內容 | `frontend/src/components/DifyChat.vue`, `frontend/src/components/AICopilot.vue` |
| S4 | DOMPurify 全域可用 | `window.DOMPurify = DOMPurify` 在 main.js 註冊 | `frontend/src/main.js`, `frontend/package.json` |

#### 二、Stage 2 — 後端模組拆分 (3 大模組)

**graph_import.py (1205 → 3 檔)**：

| 檔案 | 行數 | 職責 |
|------|------|------|
| `graph_import.py` | 205 | 薄路由層 — 匯入端點定義、向下相容 re-export |
| `import_engine.py` | 754 | 匯入核心引擎 — 智能解析、LLM 快取、合併上傳、批次處理 |
| `import_prompts.py` | 81 | LLM Prompt v2.0 模板集中管理 |

**system.py (977 → 4 檔)**：

| 檔案 | 行數 | 職責 |
|------|------|------|
| `system.py` | 271 | 薄路由層 — 組合子路由器、健康檢查、CircuitBreaker 狀態 |
| `system_config.py` | 106 | 配置工具 & Pydantic 資料模型 |
| `system_upload.py` | 206 | 通用檔案上傳管線 (Auto_Import + RAGFlow) |
| `system_connections.py` | 270 | 連線管理 CRUD 6 端點 + 自動偵測服務 |

**watcher.py (978 → 3 檔)**：

| 檔案 | 行數 | 職責 |
|------|------|------|
| `watcher.py` | 355 | Observer/Handler 框架、DLQ、重試策略 |
| `file_processor.py` | 339 | Saga 流程 / RAGFlow 上傳 / KuzuDB 寫入 / Excel 解析 |
| `node_linker.py` | 157 | 節點互連引擎：Domain 歸類 + 關鍵字共現 |

#### 三、Stage 2 — 前端模組拆分 (2 大模組)

**graphStore.js (1590 → 1012 + 3 Composables)**：

| 檔案 | 行數 | 職責 |
|------|------|------|
| `graphStore.js` | 1012 | 主 Store — 核心 state/computed/actions + composable 展開 |
| `importComposable.js` | 230 | 匯入相關 state + actions (importFile, pollImportStatus 等) |
| `crossGraphComposable.js` | 187 | 跨圖譜 state/computed/actions (aiLinks, graphMetadataList 等) |
| `ragflowComposable.js` | 28 | RAGFlow datasets 抓取 |

設計模式：Composable Factory + 依賴注入。主 Store 以 `useXxxFeatures(deps)` 初始化各模組，透過 `...spread` 組合所有回傳值。Consumer 端無需任何修改。

**GraphPage.vue (2792 → 1595 + 2 元件)**：

| 檔案 | 行數 | 職責 |
|------|------|------|
| `GraphPage.vue` | 1595 | 主頁面 — 2D/3D 切換、搜尋、篩選、事件橋接 |
| `ClusterSettingsDialog.vue` | 420 | 星系圖片設定面板 (Canvas 星球渲染、預設圖庫) |
| `NodeInspectorPanel.vue` | 371 | 節點檢查器面板 (本地編輯、Tag 管理、AI 建議連線) |

附帶清理：移除 12 個死碼函式/ref、修復 `isDraggingLeft` ref 未宣告 bug、移除未使用的 `watch` import。

#### 四、修改檔案總覽 (20 檔)

| 分類 | 檔案 | 行數 | 變更類型 |
|------|------|------|---------|
| 安全 | `backend/core/auth.py` | 328 | 修改：移除 query param auth |
| 安全 | `frontend/src/main.js` | — | 修改：DOMPurify 全域註冊 |
| 安全 | `frontend/src/components/DifyChat.vue` | — | 修改：DOMPurify sanitize |
| 安全 | `frontend/src/components/AICopilot.vue` | — | 修改：DOMPurify sanitize |
| 安全 | `frontend/package.json` | — | 修改：+dompurify ^3.3.1 |
| 後端拆分 | `backend/api/graph_import.py` | 205 | 重寫：薄路由層 |
| 後端拆分 | `backend/api/import_engine.py` | 754 | 新增：匯入核心引擎 |
| 後端拆分 | `backend/api/import_prompts.py` | 81 | 新增：Prompt 模板 |
| 後端拆分 | `backend/api/system.py` | 271 | 重寫：薄路由層 |
| 後端拆分 | `backend/api/system_config.py` | 106 | 新增：配置模型 |
| 後端拆分 | `backend/api/system_upload.py` | 206 | 新增：上傳管線 |
| 後端拆分 | `backend/api/system_connections.py` | 270 | 新增：連線 CRUD |
| 後端拆分 | `backend/services/watcher.py` | 355 | 重寫：瘦框架 |
| 後端拆分 | `backend/services/file_processor.py` | 339 | 新增：Saga 流程 |
| 後端拆分 | `backend/services/node_linker.py` | 157 | 新增：互連引擎 |
| 前端拆分 | `frontend/src/stores/graphStore.js` | 1012 | 重寫：composable 組合 |
| 前端拆分 | `frontend/src/stores/importComposable.js` | 230 | 新增：匯入 composable |
| 前端拆分 | `frontend/src/stores/crossGraphComposable.js` | 187 | 新增：跨圖譜 composable |
| 前端拆分 | `frontend/src/stores/ragflowComposable.js` | 28 | 新增：RAGFlow composable |
| 前端拆分 | `frontend/src/views/GraphPage.vue` | 1595 | 重寫：元件抽離 + 死碼清理 |
| 前端拆分 | `frontend/src/components/ClusterSettingsDialog.vue` | 420 | 新增：星系設定元件 |
| 前端拆分 | `frontend/src/components/NodeInspectorPanel.vue` | 371 | 新增：節點檢查器元件 |

**驗證結果**：
- 全部 22 個新增/修改檔案 `get_errors` 驗證：**零編譯/lint 錯誤** ✅
- 後端 `__init__.py` router 匯出未變更，`app_anytype.py` 掛載路徑不變 ✅
- 前端 consumer (NexusPage, ImportPage 等) 無需任何修改 ✅
...
**前端 Store** (`graphStore.js` 1445 → 1012 行，v5.2 拆分後)：
...
| `frontend/src/stores/graphStore.js` | 1566 → 1012 (v5.2 拆分) | Tag 過濾/統計/操作 + activeTagFilter |
